...
void anotc_parse_data(QByteArray *data)
{
    ...
    for (int i = 0; i < data->size(); i++)
    {
        if (_decode_data.status==HEAD) {
            if (data->at(i)!=ANOTC_V8_HEAD) continue; //找帧头
            ...
            _decode_data.frame.rawBytes[_decode_data.frame_index++] = data->at(i);
